...
public class CustomShulkerBullet extends Projectile {
    ...
    public CustomShulkerBullet(EntityType<? extends ShulkerBullet> p_37319_, Level p_37320_) {
    ...
    public void tick() {
        ...
        if (!this.level.isClientSide) {
            ...
            if (this.finalTarget == null || !this.finalTarget.isAlive()
                ...
                this.setDeltaMovement(vec3.add((this.targetDeltaX - vec3.x) * 0.2D, (this.targetDeltaY - vec3.y) * 0.2D,
        ...
        if (this.level.isClientSide) {
            this.level.addParticle(ParticleTypes.END_ROD, this.getX() - vec31.x, this.getY() - vec31.y + 0.15D,
                    this.getZ() - vec31.z, 0.0D, 0.0D, 0.0D);
    ...
    protected void onHitEntity(EntityHitResult p_37345_) {
        ...
        boolean flag = entity.hurt(this.damageSources().mobProjectile(this, livingentity), 7.0F);
        if (flag) {
    ...
    protected void onHitBlock(BlockHitResult p_37343_) {
        ...
        ((ServerLevel) this.level).sendParticles(ParticleTypes.EXPLOSION, this.getX(), this.getY(), this.getZ(), 2,
                0.2D, 0.2D, 0.2D, 0.0D);
    ...
    public boolean hurt(DamageSource p_37338_, float p_37339_) {
        if (!this.level.isClientSide) {
            ...
            ((ServerLevel) this.level).sendParticles(ParticleTypes.CRIT, this.getX(), this.getY(), this.getZ(), 15,
                    0.2D, 0.2D, 0.2D, 0.0D);
            this.destroy();
        }
